...
function getCurrentOrderForStoreType(orderMap, storeId, type){
  var exactKey=storeId+"_"+dateKey(type);
  if(orderMap&&orderMap[exactKey]) return orderMap[exactKey];
  var best=null;
  Object.keys(orderMap||{}).forEach(function(k){
    var o=orderMap[k];
    if(!o) return;
    if(o.store!==storeId||o.type!==type) return;
    if(!best){best=o;return;}
    var a=new Date(o.date||0).getTime(), b=new Date(best.date||0).getTime();
    if(a>b) best=o;
  });
  return best;
}
...
function AdminDash({orders,users,items,notifs,aot,setPage,stores,schedule}){
  ...
  var pendingAlerts=[];
  if(aot){stores.forEach(function(st){var o=getCurrentOrderForStoreType(orders,st.id,aot);if(!o||o.status==="draft"){var mgr=users.find(function(u){return u.storeId===st.id&&u.role==="manager"&&u.active;});pendingAlerts.push({store:st.name,manager:mgr?mgr.name:"N/A",phone:mgr?mgr.phone:"N/A"});}});}
  return(<div>
    <div style={S.sg}>
...
function MgrDash({user,orders,notifs,aot,setPage,stores,schedule,orderMsgs}){
  ...
  var curOrder=aot?getCurrentOrderForStoreType(orders,user.storeId,aot):null;
...
function OrderEntry({user,items,orders,setOrders,aot,toast,stores,schedule,orderMsgs}){
  ...
  useEffect(function(){
    setQty(function(){
      return items.reduce(function(a,it){a[it.code]=ex&&ex.items&&ex.items[it.code]!=null?ex.items[it.code]:0;return a;},{});
    });
    setNotes(function(){
      return items.reduce(function(a,it){a[it.code]=ex&&ex.notes&&ex.notes[it.code]?ex.notes[it.code]:"";return a;},{});
    });
  },[oKey,items,ex&&ex.id]);
  ...
  var applyUploadRows=function(rows){
    if(!rows||!rows.length){toast("No valid order rows found",true);return;}
    var byCode={};var byName={};
    items.forEach(function(it){byCode[normLabel(it.code)]=it;byName[normLabel(it.name)]=it;});
    var matched=0;var created=0;var skipped=0;
    var nextQ=Object.assign({},qty);
    var nextN=Object.assign({},notes);
    rows.forEach(function(r){
      var keyCode=normLabel(r.code);
      var keyName=normLabel(r.name);
      var it=(keyCode&&byCode[keyCode])||(keyName&&byName[keyName]);
      var targetCode="";
      if(it){
        targetCode=it.code;
        matched++;
      }else if(String(r.name||"").trim()){
        targetCode=syntheticOrderKeyFromName(r.name);
        created++;
      }else{
        skipped++;
        return;
      }
      nextQ[targetCode]=Math.max(0,parseInt(r.quantity,10)||0);
      nextN[targetCode]=String(r.note||"");
    });
    setQty(nextQ);
    setNotes(nextN);
    toast("Uploaded "+(matched+created)+" item rows"+(created?(" ("+created+" new names)"):"")+(skipped?(" ("+skipped+" skipped)"):""));
  };
  var handleUpload=function(e){
    ...
    if(ext==="csv"||ext==="txt"){
      ...
      r1.onload=function(ev){try{applyUploadRows(parseOrderCSV(String(ev.target.result||"")));}catch(err){console.error(err);toast("Could not parse CSV",true);}};
      r1.readAsText(f);
      ...
      r2.onload=function(ev){
        try{
          ...
          if(!ws){toast("Excel sheet not found",true);return;}
          var rows=XLSX.utils.sheet_to_json(ws,{header:1,raw:false,defval:"",blankrows:false});
          var parsed=parseOrderSheetRows(rows);
          if(!parsed.length){
            parsed=parseOrderSheetRows(worksheetToRows(ws));
          }
          if(!parsed.length){
            toast("Excel format not recognized. Expected headers: Item Name, Qty, Note",true);
            return;
          }
          applyUploadRows(parsed);
        }catch(err){console.error(err);toast("Could not parse Excel file: "+(err&&err.message?err.message:"Unknown error"),true);}
      };
      r2.readAsArrayBuffer(f);
    }else{
      toast("Use CSV or Excel (.xlsx)",true);
    }
    e.target.value="";
  };
  ...
  var saveAndSendDraft=async function(){
    try{
      const resp = await apiClient.orders.create({type:sel,items:qty,notes:notes,status:"draft_shared",storeId:user.storeId});
      var id = resp.orderId;
      setOrders(function(p){var n=Object.assign({},p);n[oKey]={id:id,items:Object.assign({},qty),notes:Object.assign({},notes),status:"draft_shared",store:user.storeId,type:sel,date:new Date().toISOString()};return n;});
      toast("Draft sent to admin");
    }catch(e){toast(e.message,true);}
  };
  ...
  var sorted=useMemo(function(){
    ...
    return known.concat(extras).sort(function(a,b){if(sortBy==="name")return a.name.localeCompare(b.name);if(sortBy==="code")return String(a.code).localeCompare(String(b.code));var c=(a.category||"").localeCompare(b.category||"");return c!==0?c:a.name.localeCompare(b.name);});
  },[items,sortBy,qty,notes]);
  return(<div>
    ...
    <div style={S.card}><div style={S.cH}>
      ...
      <div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}>
        {!ro&&(<Fragment><button style={Object.assign({},S.b,S.bS)} onClick={function(){upRef.current&&upRef.current.click();}}>Upload CSV/Excel</button><button style={Object.assign({},S.b,S.bS)} onClick={save}>Save Draft</button><button style={Object.assign({},S.b,S.bP)} onClick={function(){setShowConfirm(true);}}>Submit</button></Fragment>)}
        ...
        <input ref={upRef} type="file" accept=".csv,.txt,.xls,.xlsx" style={{display:"none"}} onChange={handleUpload}/>
      </div>
    </div>
    <div style={Object.assign({},S.card,{padding:"10px 14px"})}>
      <div style={{display:"flex",gap:16,flexWrap:"wrap",alignItems:"center"}}>
        <div style={{fontSize:12.5}}><span style={{color:"#9BA1B5"}}>Total Cases:</span> <strong style={{color:"#E8EAF0",fontFamily:"monospace"}}>{totalCases}</strong></div>
      </div>
    </div>
...
/* ═══ ORDER HISTORY ═══ */
function OrderHistory({user,orders,items,setOrders,toast}){
  ...
  var sendDraftFromHistory=async function(k,o){
    try{
      const resp=await apiClient.orders.create({type:o.type,items:o.items||{},notes:o.notes||{},status:"draft_shared",storeId:user.storeId});
      var id=resp.orderId;
      setOrders(function(p){var n=Object.assign({},p);n[k]=Object.assign({},n[k]||o,{id:id,status:"draft_shared"});return n;});
      toast("Draft sent to admin");
    }catch(e){toast(e.message,true);}
  };
...
function Consolidated({orders,setOrders,items,aot,toast,stores,consolidatedType,setConsolidatedType}){
  ...
  var startE=function(sid){
    ...
    var exItems=exOrder.items||{};
    var exNotes=exOrder.notes||{};
    var q={};var n={};
    items.forEach(function(it){q[it.code]=exItems[it.code]||0;n[it.code]=exNotes[it.code]||"";});
    sEQ(q);sENotes(n);sES(sid);
  };
  var saveE=async function(){
      if(!eSt) return;
      try{
        const resp = await apiClient.orders.create({type:vt,items:eQ,notes:eNotes,status:"submitted",storeId:eSt});
        const id = resp.orderId;
        var k=eSt+"_"+dk;
        setOrders(function(p){var n=Object.assign({},p);n[k]=Object.assign({},p[k]||{},{id:id,items:Object.assign({},eQ),notes:Object.assign({},eNotes),status:(p[k]||{}).status||"submitted",store:eSt,type:vt,date:(p[k]||{}).date||new Date().toISOString()});return n;});
        toast("Updated");
        sES(null);sEQ({});sENotes({});
      }catch(e){toast(e.message,true);}  };
  var cancelE=function(){sES(null);sEQ({});sENotes({});};
  ...
  return(<div>
    ...
    <div style={Object.assign({},S.card,{padding:0})}>
      ...
      <div style={Object.assign({},S.tw,{border:"none",borderRadius:0})}><table style={S.tbl}><thead><tr><th style={S.th}>Item Name</th>
        {stores.map(function(st){return(<Fragment key={st.id}><th style={Object.assign({},S.th,{textAlign:"center",minWidth:90})}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:2}}><span>{st.name.split(" ")[0]} Qty</span><button disabled={isFinished} style={Object.assign({},S.eB,eSt===st.id?{color:"#4F8CFF"}:{},isFinished?{opacity:0.4,cursor:'not-allowed'}:{})} onClick={function(){if(isFinished) return; eSt===st.id?cancelE():startE(st.id);}}><Ic type="edit" size={11}/></button></div></th><th style={Object.assign({},S.th,{minWidth:160})}>{st.name.split(" ")[0]} Note</th></Fragment>);})}<th style={Object.assign({},S.th,{textAlign:"center",background:"#272B38"})}>Total Cases</th></tr></thead>
        <tbody>{(function(){
          var knownCodes={};items.forEach(function(it){knownCodes[it.code]=true;});
          var dynamicCodes={};
          stores.forEach(function(st){
            var o=getStoreOrder(st.id)||{};
